...
Step-by-step Circom circuit setup and generation of keys/proof for the `bls12_381_verifier` fixture. Run the commands in order.

```sh
cd contracts/bls12_381_verifier/tests/data/circom/auxiliary
...
# Export verification key (used by the fixture)
...
After running, `contracts/bls12_381_verifier/tests/data/circom/` will contain `verification_key.json`, `proof.json`, and `public.json` - use them to update the fixture and integration tests.
